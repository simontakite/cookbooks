...
class Basket {
	...
	public function checkBasket()
	{
		...
		{
			//echo 'set';
			$this->deliveryAddress = unserialize($_SESSION['delivery_address']);
		...
		elseif( $this->registry->getObject('authenticate')->isLoggedIn() )
		{
			...
			$this->setDeliveryAddress( $d['name'], $d['address_lineone'], $d['address_linetwo'], $d['address_city'], $d['address_postcode']);
		}
	...
	public function setDeliveryAddress( $name, $line1, $line2, $city, $postcode, $country )
	{
		...
		$_SESSION['delivery_address'] = serialize( $this->deliveryAddress );
	}
