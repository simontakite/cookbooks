...
#  		Notes    	
# This script is used for an Openstack Disaster Recovery Process. The script will be used every time there is a connection loose between 
# the cloud controller and the compute-node.
...
# Binaries
	...
	SED=/bin/sed
# Misc
# The test moade allows the whole process to run to a test instance only. The instance id has to be set into the "instance_test_id" variable. 
# It supposes that you have created a dummy instances and attached an EBS volume to it.
	test_mode=1
