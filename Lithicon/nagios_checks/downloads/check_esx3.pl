#!/usr/bin/perl -w
# Edit history:
## 30.03.2010 - torad: Updated to handle case variation in the value "Green/green" in ESXi 3.5 / 4.0 respectively.
### + if (uc($_->healthState->key) ne 'GREEN')
...
sub host_list_vm_volumes_info
{
	...
	if (defined($subcommand))
	{
		...
		foreach my $ref_store (@{$host_view->datastore})
		{
			...
			{
				$res = 'OK';
				# Haxx: it worked like this: if free% < w% then ok, which is totally retarded. We'd want if free% > w% then ok.
				# Fixed by inverting the numbers checked (used space instead of free space), while displaying the original output.
				# -torad 24.07.2009 15:57
				my $valuea=simplify_number((convert_number($store->summary->capacity)-convert_number($store->summary->freeSpace))/1024/1024);
				my $valueb = simplify_number(100-(convert_number($store->info->freeSpace) / convert_number($store->summary->capacity)*100));
				# /Haxx
				...
				if ($perc)
				{
					$res = $np->check_threshold(check => $valueb);
				}
				else
				{
					$res = $np->check_threshold(check => $valuea);
				}				
				#$output = $store->summary->name . "=". $value1 . " MB (" . $value2 . "%)";
				$output = "Free space: " . $store->summary->name . " = ". $value1 . " MB (" . $value2 . "%)";
			}
		}
	}
	else
...
sub host_runtime_info
{
	...
	else
	{
		...
		if (defined($cpuStatusInfo))
		{
			...
			{
				$SensorCount++;
				$AlertCount++ if (uc($_->status->key) ne 'GREEN');
			}
		}
		...
		if (defined($numericSensorInfo))
		{
			...
			{
				$SensorCount++;
				$AlertCount++ if (uc($_->healthState->key) ne 'GREEN');
			}
		}
